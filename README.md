...
- `usage_monthly`：过期自然月清理前生成的月总量摘要，供历史页面和 `/api/v1/stats/monthly` 查询。
...
每次清理会先把过期月份写入 `usage_monthly`，再删除该月的分钟、小时、链路和日志证据明细。这样可以保留自然月总量，同时把完整明细限制在三个月窗口内。前端 `History` 页面会把保留期内月份和已归档月份放在同一张月度表里：保留期内月份可继续跳转明细，已归档月份只展示月总量、转发总量、证据数和链路数。
